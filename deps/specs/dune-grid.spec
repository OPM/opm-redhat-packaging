...
Requires:       libdune-grid2.11 = %{version}
...
%package -n libdune-grid2.11
Summary:        Grid management module for DUNE
Group:          System/Libraries
Obsoletes:      libdune-grid

%description -n libdune-grid2.11
dune-grid defines nonconforming, hierarchically nested, multi-element-type,
parallel grids in arbitrary space dimensions. Graphical output with several
packages is available, e.g. file output to IBM data explorer and VTK (parallel
XML format for unstructured grids). The graphics package Grape has been integrated
...
%if 0%{?_build_openmpi}
%package -n libdune-grid2.11-openmpi
Summary:        Grid management module for DUNE - openmpi version
Group:          System/Libraries
Obsoletes:      libdune-grid-openmpi

%description -n libdune-grid2.11-openmpi
dune-grid defines nonconforming, hierarchically nested, multi-element-type,
parallel grids in arbitrary space dimensions. Graphical output with several
packages is available, e.g. file output to IBM data explorer and VTK (parallel
XML format for unstructured grids). The graphics package Grape has been integrated
...
Group:          Development/Libraries/C and C++
Requires:       libdune-grid2.11-openmpi = %{version}
...
%if 0%{?_build_mpich}
%package -n libdune-grid2.11-mpich
Summary:        Grid management module for DUNE - mpich version
Group:          System/Libraries
Obsoletes:      libdune-grid-mpich

%description -n libdune-grid2.11-mpich
dune-grid defines nonconforming, hierarchically nested, multi-element-type,
parallel grids in arbitrary space dimensions. Graphical output with several
packages is available, e.g. file output to IBM data explorer and VTK (parallel
XML format for unstructured grids). The graphics package Grape has been integrated
...
Group:          Development/Libraries/C and C++
Requires:       libdune-grid2.11-mpich = %{version}
...
%post -n libdune-grid2.11 -p /sbin/ldconfig
%postun -n libdune-grid2.11 -p /sbin/ldconfig

%if 0%{?_build_openmpi}
%post -n libdune-grid2.11-openmpi -p /sbin/ldconfig
%postun -n libdune-grid2.11-openmpi -p /sbin/ldconfig
%endif

%if 0%{?_build_mpich}
%post -n libdune-grid2.11-mpich -p /sbin/ldconfig
%postun -n libdune-grid2.11-mpich -p /sbin/ldconfig
%endif
...
%if 0%{?_build_openmpi}
%files -n libdune-grid2.11-openmpi
%defattr(-,root,root,-)
...
%if 0%{?_build_mpich}
%files -n libdune-grid2.11-mpich
%defattr(-,root,root,-)
